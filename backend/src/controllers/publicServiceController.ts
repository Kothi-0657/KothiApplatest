...
export const listServices = async (req: Request, res: Response) => {
  try {
    const result = await pool.query("SELECT id, category, name, price, sequence, icon, created_at, updated_at FROM services ORDER BY sequence ASC, created_at DESC");
...
export const getService = async (req: Request, res: Response) => {
  try {
    const { rows } = await pool.query("SELECT id, category, name, price, sequence, icon, created_at, updated_at FROM services WHERE id = $1 LIMIT 1", [req.params.id]);
