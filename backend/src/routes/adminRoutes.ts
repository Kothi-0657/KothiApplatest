...
router.get("/stats", authenticate, async (_req: Request, res: Response) => {
  try {
    const [customers, vendors, bookings, services, payments, revenue] = await Promise.all([
      ...
      pool.query("SELECT COALESCE(SUM(amount), 0)::numeric AS total FROM payments WHERE status = 'success'")
    ]);
...
router.put("/customers/:id/status", authenticate, async (req: Request, res: Response) => {
  try {
...
router.get("/bookings/:id", authenticate, async (req: Request, res: Response) => {
  try {
    const { rows } = await pool.query(
      `
      SELECT
...
router.patch("/bookings/:id/status", authenticate, async (req: Request, res: Response) => {
  try {
    const { status } = req.body;
...
router.patch("/bookings/:id/vendor", authenticate, async (req: Request, res: Response) => {
  try {
    ...
    if (v.rowCount === 0) return res.status(404).json({ success: false, message: "Vendor not found" });

    const { rows } = await pool.query(
      `UPDATE bookings SET vendor_id = $1, updated_at = NOW() WHERE id = $2 RETURNING *`,
...
router.get("/vendors/:id", authenticate, async (req: Request, res: Response) => {
  try {
    ...
    if (rows.length === 0) return res.status(404).json({ success: false, message: "Vendor not found" });
...
router.post("/vendors", authenticate, async (req: Request, res: Response) => {
  try {
    ...
    if (!company_name) return res.status(400).json({ success: false, message: "company_name required" });
...
router.put("/vendors/:id/status", authenticate, async (req: Request, res: Response) => {
  try {
    const { status } = req.body;
    const { rows } = await pool.query(`UPDATE vendors SET status = $1, updated_at = NOW() WHERE id = $2 RETURNING *`, [status, req.params.id]);
    if (rows.length === 0) return res.status(404).json({ success: false, message: "Vendor not found" });
    res.json({ success: true, vendor: rows[0] });
  } catch (err) {
...
router.delete("/vendors/:id", authenticate, async (req: Request, res: Response) => {
  try {
    ...
    if (rowCount === 0) return res.status(404).json({ success: false, message: "Vendor not found" });
...
router.get("/payments/:id", authenticate, async (req: Request, res: Response) => {
  try {
    const { rows } = await pool.query(
      `SELECT p.*, b.booking_ref FROM payments p LEFT JOIN bookings b ON p.related_booking = b.id WHERE p.id = $1 LIMIT 1`,
      [req.params.id]
    );
...
// Update payment (full)
router.put("/payments/:id", authenticate, async (req: Request, res: Response) => {
  try {
...
router.put("/payments/:id/status", authenticate, async (req: Request, res: Response) => {
  try {
    const { status } = req.body;
    const { rows } = await pool.query(`UPDATE payments SET status = $1, updated_at = NOW() WHERE id = $2 RETURNING *`, [status, req.params.id]);
    if (rows.length === 0) return res.status(404).json({ success: false, message: "Payment not found" });
    res.json({ success: true, payment: rows[0] });
  } catch (err) {
...
router.put("/payments/:id/verify", authenticate, async (req: Request, res: Response) => {
  try {
    const { transaction_id } = req.body;
    const { rows } = await pool.query(
      `UPDATE payments SET status = 'success', transaction_id = COALESCE($1, transaction_id), updated_at = NOW() WHERE id = $2 RETURNING *`,
...
// DELETE payment
router.delete("/payments/:id", authenticate, async (req: Request, res: Response) => {
  try {
    const { rowCount } = await pool.query(`DELETE FROM payments WHERE id = $1`, [req.params.id]);
    if (rowCount === 0) return res.status(404).json({ success: false, message: "Payment not found" });
...
router.post("/services", authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO services (name, description, category, image_url, created_at) VALUES ($1,$2,$3,$4,NOW()) RETURNING *`,
...
router.put("/services/:id", authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const { name, description, category, image_url } = req.body;
    const { rows } = await pool.query(
      `UPDATE services SET name = $1, description = $2, category = $3, image_url = $4, updated_at = NOW() WHERE id = $5 RETURNING *`,
      [name, description || null, category || null, image_url || null, id]
    );
...
router.delete("/services/:id", authenticate, async (req: Request, res: Response) => {
  try {
    ...
    if (rowCount === 0) return res.status(404).json({ success: false, message: "Service not found" });
...
router.get("/search", authenticate, async (req: Request, res: Response) => {
  try {
    ...
    if (!type || !q) return res.status(400).json({ success: false, message: "Missing search parameters" });
